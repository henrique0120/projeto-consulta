...
import { PatientScheduleAppointmentModel, SaveScheduleModel, ScheduleAppointementMonthModel, SelectPatientModel } from '../../schedule.models';
...
export class ScheduleCalendarComponent implements OnDestroy, AfterViewInit, OnChanges {
  ...
  displayedColumns: string[] = ['startAt', 'endAt', 'patient', 'actions'];

  dataSource!: MatTableDataSource<PatientScheduleAppointmentModel>

  addingSchedule: boolean = false

  newSchedule: SaveScheduleModel = { startAt: undefined, endAt: undefined, patientId: undefined }

  patientsSelectFormControl = new FormControl()

  @Input() monthSchedule!: ScheduleAppointementMonthModel
  @Input() patients: SelectPatientModel[] = []

  @Output() onDateChange = new EventEmitter<Date>()
  @Output() onConfirmDelete = new EventEmitter<PatientScheduleAppointmentModel>()
  @Output() onSchedulePatient = new EventEmitter<SaveScheduleModel>()
  ...
  onSubmit(form: NgForm) {
    ...
    const saved: PatientScheduleAppointmentModel = {
      id: -1,
      day: this._selected.getDate(),
      startAt,
      endAt,
      patientId: this.newSchedule.patientId!,
      patientName: this.patients.find(c => c.id === this.newSchedule.patientId!)!.name
    }
    this.monthSchedule.scheduledAppointments.push(saved)
    this.onSchedulePatient.emit(saved)
    this.buildTable()
    form.resetForm()
    this.newSchedule = { startAt: undefined, endAt: undefined, patientId: undefined }
  }

  requestDelete(schedule: PatientScheduleAppointmentModel) {
    this.subscription = this.dialogManagerService.showYesNoDialog(
      ...
      if (result) {
        ...
        this.dataSource = new MatTableDataSource<PatientScheduleAppointmentModel>(updatedeList)
        if (this.paginator) {
          this.dataSource.paginator = this.paginator
        }
      }
  ...
  private buildTable() {
    ...
    this.dataSource = new MatTableDataSource<PatientScheduleAppointmentModel>(appointments);
    if (this.paginator) {
      this.dataSource.paginator = this.paginator
    }
  }
